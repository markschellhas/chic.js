...
export function routeServerIndexPageTemplate(resourceName) {
    ...
    return `
import { ${dbMethodName} } from '$lib/controllers/${resourceName}/${resourceName}_controller';
...
export function routeServerNewPageTemplate(resourceName) {
    const dbMethodName = `create${capitalize(singularize(resourceName))}`;
    const resourceNameSingular = singularize(resourceName);
    return `
import { ${dbMethodName} } from '$lib/controllers/${resourceName}/${resourceName}_controller';
import { fail, redirect } from '@sveltejs/kit';
...
export function routeServerEditPageTemplate(resourceName) {
    const dbMethodName = `edit${capitalize(singularize(resourceName))}`;
    const dbGetMethodName = `get${capitalize(singularize(resourceName))}`;
    const resourceNameSingular = singularize(resourceName);
    return `
import { ${dbMethodName}, ${dbGetMethodName} } from '$lib/controllers/${resourceName}/${resourceName}_controller';
import { fail, redirect } from '@sveltejs/kit';
...
export function APIItemRouteTemplate(resourceName) {
    const methodName = `get${capitalize(singularize(resourceName))}`;
    return `
...
export async function GET({ params  }) {
    const { id } = params;
    const ${singularize(resourceName)} = await ${methodName}(id);
