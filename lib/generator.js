...
import { inspectDatabase, parseTableList, prepareScaffoldTables } from './introspect.js';
...
export function namesFor(input, options = {}) {
  ...
  return {
    ...
    table: options.table || snake(plural)
  };
}
...
function drizzleColumn(field) {
  ...
  switch (field.type) {
    ...
    case 'datetime':
      ...
      break;
    case 'references': {
      const column = /(_id|Id)$/.test(field.column) ? field.column : `${field.column}_id`;
      expression = `integer('${column}').references(() => ${field.reference}.id)`;
      break;
    }
    case 'json':
...
function schemaTemplate(names, fields, options = {}) {
...
function serviceTemplate(names, fields, options = {}) {
...
export async function update${names.model}(id: number, input: unknown) {
  ...
  return db.update(${names.plural})
    .set({ ...data${options.updatedAt !== false ? ', updatedAt: new Date()' : ''} })
...
export function buildScaffoldPlan(root, resource, fieldValues, options = {}) {
  const names = namesFor(resource, { table: options.tableName });
  const fields = Array.isArray(options.fields) ? options.fields : parseFields(fieldValues);
  ...
  if (
    existingModel
    && !options.allowFieldChange
    && JSON.stringify(existingModel.fields) !== JSON.stringify(fields)
  ) {
    throw new Error(
  ...
  const skipMigration = Boolean(options.skipMigration);
  ...
  if (!skipMigration) {
    artifacts.push(`${migrationBase}.up.sql`, `${migrationBase}.down.sql`);
  }
  ...
  const schemaIndex = updateSchemaIndex(root, names, false, options.schemaIndexContent);
  const migrations = migrationTemplates(names, fields);
  plan.write(`src/lib/server/db/schema/${names.plural}.ts`, schemaTemplate(names, fields, timestampOptions));
  plan.write(schemaIndex.relativePath, schemaIndex.content, { overwrite: true });
  plan.write(`src/lib/server/${names.plural}.ts`, serviceTemplate(names, fields, timestampOptions));
  ...
  return { plan, names, config: nextConfig, schemaIndexContent: schemaIndex.content };
}

export async function buildFromDatabasePlan(root, options = {}) {
  const config = options.config || readConfig(root);
  if (config.orm !== 'drizzle') {
    throw new Error('This generator requires a Drizzle Chic project. Run `chic upgrade` first.');
  }

  const inspected = await inspectDatabase(root, options);
  const { prepared, skipped } = prepareScaffoldTables(inspected, options);
  if (!prepared.length) {
    const extra = skipped.length ? ` ${skipped.map((entry) => entry.skipReason).join(' ')}` : '';
    throw new Error(`No tables to scaffold from ${inspected.path}.${extra}`);
  }

  const combined = new GenerationPlan(root, options);
  let nextConfig = config;
  let schemaIndexContent;
  const summaries = [];
  const onlyRequested = parseTableList(options.only).length > 0;

  for (const table of prepared) {
    const names = namesFor(table.resource, { table: table.name });
    const existingModel = nextConfig.models.find((model) => model.name === names.model);
    if (existingModel && !options.force) {
      if (onlyRequested) {
        throw new Error(`${existingModel.name} already exists. Use --force to regenerate it.`);
      }
      console.log(`  skip   ${existingModel.name} (already registered; use --force to regenerate)`);
      continue;
    }

    const built = buildScaffoldPlan(root, table.resource, null, {
      ...options,
      fields: table.fields,
    ...
    schemaIndexContent = built.schemaIndexContent;
    for (const operation of built.plan.operations) {
      if (
        operation.relativePath === 'chic.json'
        || operation.relativePath === 'src/routes/__chic/routes/+server.ts'
      ) {
        continue;
      }
      combined.operations.push(operation);
    }
    summaries.push({ names: built.names, table: table.name, fields: table.fields });
  }

  if (!summaries.length) {
    throw new Error('No tables to scaffold. Registered models were skipped; use --force to regenerate them.');
  }

  combined.write('src/routes/__chic/routes/+server.ts', routeInspector(nextConfig), { overwrite: true });
  combined.write('chic.json', `${JSON.stringify(nextConfig, null, 2)}\n`, { overwrite: true });
  return {
    plan: combined,
    config: nextConfig,
    tables: summaries,
    skipped,
    database: inspected.path
  };
}
