  ...
  async execute() {
    ...
    try {
      for (const operation of this.operations) {
        const absolutePath = path.join(this.root, operation.relativePath);
      ...
      for (const item of applied.reverse()) {
        if (item.previous === null) fs.rmSync(item.absolutePath, { force: true, recursive: true });
        else {
          fs.mkdirSync(path.dirname(item.absolutePath), { recursive: true });
          fs.writeFileSync(item.absolutePath, item.previous);
        }
      }
      throw error;
    }
...
function zodExpression(field) {
  ...
  switch (field.type) {
    ...
    case 'datetime':
      ...
      break;
    case 'json':
      expression = `z.preprocess((value) => typeof value === 'string' ? JSON.parse(value) : value, z.unknown())`;
      break;
    default:
...
export function routeInspector(config) {
  const rows = config.routes.map((route) => `<tr><td><code>${route.method}</code></td><td><a href="${route.path}">${route.path}</a></td><td>${route.description || ''}</td><td><code>${route.file || ''}</code></td></tr>`).join('');
...
export function buildScaffoldPlan(root, resource, fieldValues, options = {}) {
  ...
  if (config.models.some((model) => model.name === names.model) && !options.force) {
  ...
  const migrationBase = `drizzle/chic/${stamp}_create_${names.table}`;
  const artifacts = [
...
export function buildDestroyPlan(root, resource, options = {}) {
  ...
  const plan = new GenerationPlan(root, options);
  for (const artifact of model.artifacts || []) plan.remove(artifact);
