...
export function createFormComponent(modelName, paths, fields) {
    ...
    const componentContent = generateFormComponentContent(fields);
    writeFormComponent(componentContent, name);
}

/**
 * Generates a Svelte form component for a space-delimited field specification.
 * @param {string} fields - The fields to include in the form.
 * @returns {string} The generated Svelte component.
 */
export function generateFormComponentContent(fields) {
    const formFieldsSpec = transformFieldsToObject(fields);
    let formFields = addHtmlFields(formFieldsSpec);
    let formHtml = generateHtmlForm(formFields);
    return insertForm(formComponentTemplate, formHtml);
}
...
function addHtmlFields(fields) {
    return fields.map(field => {
        ...
        switch (field.type) {
            case 'string':
                html = `<input id="${field.name}" name="${field.name}" bind:value={data.${field.name}} type="text" />`;
                break;
            case 'text':
                html = `<textarea id="${field.name}" name="${field.name}" bind:value={data.${field.name}}></textarea>`;
                break;
            case 'number':
                html = `<input id="${field.name}" name="${field.name}" bind:value={data.${field.name}} type="number" />`;
                break;
            case 'file':
                html = `<input id="${field.name}" name="${field.name}" type="file" />`;
                break;
            default:
                html = `<input id="${field.name}" name="${field.name}" bind:value={data.${field.name}} type="text" />`;
                break;
...
function generateHtmlForm(fields) {
    ...
    const encoding = fields.some((field) => field.type === 'file') ? ' enctype="multipart/form-data"' : '';
