...
export async function installDrizzle(root, packageManager = detectPackageManager(root)) {
  ...
  if (!allDependencies['drizzle-orm']) {
    await run('npx', [
      'sv', 'add', 'drizzle=database:sqlite+client:better-sqlite3',
...
export async function initializeProject(root, options = {}) {
  ...
  const svelteConfigExists = ['svelte.config.js', 'svelte.config.ts'].some((file) =>
    fs.existsSync(path.join(root, file))
  );
  if (!svelteConfigExists) throw new Error('This does not look like a SvelteKit project.');
...
`);
  ...
  if (!fs.existsSync(path.join(root, 'src/lib/server/db/schema.ts'))) {
...
export async function createProject(name, options = {}) {
  ...
  const addons = ['drizzle=database:sqlite+client:better-sqlite3'];
  ...
  await initializeProject(root, { noInstall: false, packageManager, force: true });
  return root;
}
