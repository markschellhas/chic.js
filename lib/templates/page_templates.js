    ...
    new: async ({ request }) => {
        ...
        return ${createMethodName}(body)
            ...
            .catch(err => {
                ...
                return fail(500, {
                    message: err instanceof Error ? err.message : 'Failed to create ${resourceNameSingular}'
                });
            });
    },
    destroy: async ({ request }) => {
        ...
        return ${destroyMethodName}(body)
            ...
            .catch(err => {
                ...
                return fail(500, {
                    message: err instanceof Error ? err.message : 'Failed to destroy ${resourceNameSingular}'
                });
            });
    }
};
    ...
    update: async ({ request }) => {
        ...
        return ${updateMethodName}(body)
            ...
            .catch(err => {
                ...
                return fail(500, {
                    message: err instanceof Error ? err.message : 'Failed to update ${resourceNameSingular}'
                });
            });
    }
};
...
export function mainContentOnIndexPage(resourceName, firstFieldName) {
    return `
    <ul>
    {#each data.${resourceName} as item (item.id)}
      <li>
...
export function mainContentOnShowPage(resourceName) {
    return `
    {#each Object.entries(data.${singularize(resourceName)}) as [key, val] (key)}
      <p><b>{key}:</b> {val}</p>
    {/each}
`;
}
