...
export function APIItemRouteTemplate(resourceName) {
    ...
    const destroyMethodName = `destroy${capitalize(singResourceWord)}`;

    return `
import { ${methodName}, ${updateMethodName}, ${destroyMethodName} } from '$lib/controllers/${resourceName}/${resourceName}_controller';
import { json } from '@sveltejs/kit';
...
export async function PUT({ params, request  }) {
    const { id } = params;
    const body = await request.json();
    const ${singularize(resourceName)} = await ${updateMethodName}({ ...body, id });
...
export async function DELETE({ params }) {
    const { id } = params;
    const ${singularize(resourceName)} = await ${destroyMethodName}(id);
    return json(${singularize(resourceName)});
}
