...
import { controllerTemplate, linksOnIndexPageTemplate, linksOnShowPageTemplate, mainContentOnIndexPage, stylesOnFormPageTemplate, mainContentOnShowPage } from "./templates.js";
...
export function contentReplaceNewFormPage(content, resourceName, path) {
    content = content.replace('{FORM_IMPORT}', "import Form from '$lib/components/" + resourceName + "/Form.svelte';");
    content = content.replace('{DATA_IMPORT}', "");
    content = content.replace('{FORM_PLACEMENT}', "<Form />");
...
export function contentReplaceEditFormPage(content, resourceName, path) {
    content = content.replace('{FORM_IMPORT}', "import Form from '$lib/components/" + resourceName + "/Form.svelte';");
    content = content.replace('{DATA_IMPORT}', "export let data;");
    content = content.replace('{FORM_PLACEMENT}', `<Form data={data.${singularize(resourceName)}} />`);
    content = content.replace('{MAIN_CONTENT}', "");
    content = content.replace('{STYLES}', stylesOnFormPageTemplate);
    content = content.replace('{LINKS}', `<a href="/${resourceName}">Back to ${resourceName}</a>`);
    return content;
